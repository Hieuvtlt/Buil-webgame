...
function hudMarkup(){const{stats,hp,mp}=getHudStats();const hpPct=stats.maxHp?Math.max(0,Math.min(100,hp/stats.maxHp*100)):0;const mpPct=stats.maxMp?Math.max(0,Math.min(100,mp/stats.maxMp*100)):0;const expPct=player.expToNextLevel?Math.max(0,Math.min(100,player.exp/player.expToNextLevel*100)):0;const nav=menuNames.map((name,index)=>`<button class="menu-item${index===0?' active':''}" type="button" data-screen="${name}"><span class="menu-icon">${['◈','♙','◉','⚗','⚒','◆','▣','✦','◌','⚙','☷'][index]??'•'}</span><span>${name}</span></button>`).join('');return `<aside id="col-left"><section class="profile-card"><div class="profile-head"><img src="${characterImg}" alt="Nhân vật" class="profile-avatar"/><div class="profile-info"><div class="profile-name">${player.name}</div><div class="profile-level">Lv. ${player.level}</div><div class="profile-exp">${player.exp} / ${player.expToNextLevel} (${expPct.toFixed(2)}%)</div><div class="hud-progress exp"><span style="width:${expPct}%"></span></div></div></div><div class="resource-row"><span>HP</span><strong>${hp}/${stats.maxHp}</strong><div class="hud-progress hp"><span style="width:${hpPct}%"></span></div></div><div class="resource-row"><span>MP</span><strong>${mp}/${stats.maxMp}</strong><div class="hud-progress mp"><span style="width:${mpPct}%"></span></div></div></section><nav id="left-menu" class="main-menu">${nav}</nav><section class="left-stats-card"><h3>THUỘC TÍNH</h3><div class="left-stat"><span>Sức mạnh</span><b>${stats.strength}</b></div><div class="left-stat"><span>Thân pháp</span><b>${stats.dexterity}</b></div><div class="left-stat"><span>Sinh khí</span><b>${stats.vitality}</b></div><div class="left-stat"><span>Nội lực</span><b>${stats.energy}</b></div><div class="left-stat"><span>Chính xác</span><b>${stats.accuracy}</b></div><div class="left-stat"><span>Né tránh</span><b>${stats.dodge}</b></div><div class="left-stat resistance"><span>Kháng độc</span><b>${stats.poisonResistance}%</b></div><div class="left-stat resistance"><span>Kháng hỏa</span><b>${stats.fireResistance}%</b></div><div class="left-stat resistance"><span>Kháng băng</span><b>${stats.iceResistance}%</b></div><div class="left-stat resistance"><span>Kháng lôi</span><b>${stats.lightningResistance}%</b></div></section></aside>`}

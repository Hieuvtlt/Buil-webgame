...
const NPCS=[
  {id:'hunter',name:'Lão Thợ Săn',role:'NPC',x:470,y:900,icon:'🏹',text:'Có dấu chân thú lớn ở phía bắc. Hãy cẩn thận khi vào rừng sâu.'},
...
function injectStyle(){
  ...
  const style=document.createElement('style');style.id='world-content-v1-style';style.textContent=`
    ...
    .world-npc-node{position:absolute;transform:translate(-50%,-100%);width:86px;text-align:center;pointer-events:auto;cursor:pointer;filter:drop-shadow(0 3px 3px rgba(0,0,0,.7))}
...
function mountContent(){
  ...
  NPCS.forEach(n=>{const el=document.createElement('button');el.type='button';el.className='world-npc-node';el.style.left=n.x+'px';el.style.top=n.y+'px';el.innerHTML=`<span class="npc-icon">${n.icon}</span><b>${n.name}</b><small>${n.role}</small>`;el.addEventListener('click',e=>{e.stopPropagation();openToast(root,`${n.name}: ${n.text}`)});layer.appendChild(el)})

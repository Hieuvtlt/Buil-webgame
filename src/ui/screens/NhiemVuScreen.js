...
const monsterTemplates = [
  ...
  { name:'Dẹp Sơn Tặc', map:'Sơn Tặc Sơn', target:'Sơn Tặc', min:1, max:40, baseCount:36 },
  { name:'Trừ Yêu Xà', map:'Thanh Xà Cốc', target:'Thanh Xà Yêu', min:11, max:50, baseCount:42 },
  { name:'Quét Địa Huyệt', map:'Địa Huyệt', target:'Địa Huyệt Trùng', min:21, max:60, baseCount:48 },
  { name:'Dẹp Ma Binh', map:'Ma Binh Cốc', target:'Ma Binh', min:31, max:70, baseCount:55 },
  { name:'Săn Hắc Hổ', map:'Hắc Hổ Lâm', target:'Hắc Hổ', min:41, max:90, baseCount:70 },
  { name:'Thanh Trừ Quỷ Tướng', map:'U Minh Cốc', target:'Quỷ Tướng', min:51, max:100, baseCount:80 },
  { name:'Diệt Huyết Ma', map:'Huyết Ma Sơn', target:'Huyết Ma', min:61, max:120, baseCount:90 },
  { name:'Quét Thiên Yêu', map:'Thiên Yêu Lĩnh', target:'Thiên Yêu', min:81, max:200, baseCount:200 },
...
function monsterView(state){
  ...
  return `<div class="quest-summary"><span>⚔ Đang nhận: <b>${activeIds.size}/2</b></span><span>Nhận nhiệm vụ cả ngày · Không giới hạn số lần nhận</span></div><div class="quest-list" role="list">${quests.map((q,i)=>{const active=activeIds.has(i),disabled=active||activeIds.size>=2||!q.suitable;return `<article class="quest-card${!q.suitable?' quest-not-suitable':''}" role="listitem"><div class="quest-avatar">⚔</div><div class="quest-body"><div class="quest-name">${q.name}</div><div class="quest-meta">📍 ${q.map} · Lv.${q.min}–${q.max}<br>🎯 ${q.target} · <b>${q.count} con</b></div><div class="quest-progress">Tiêu diệt: <strong>0/${q.count}</strong></div><div class="quest-reward">🎁 ${q.reward}</div></div><div class="quest-choice">${q.suitable?`<span class="quest-suitable">Phù hợp Lv. ${player.level}</span>`:'<span class="quest-unsuitable">Ngoài cấp độ</span>'}<button class="quest-action" type="button" data-type="monster" data-index="${i}" ${disabled?'disabled':''}>${active?'ĐANG NHẬN':activeIds.size>=2&&q.suitable?'ĐÃ ĐỦ 2 NHIỆM VỤ':'NHẬN NHIỆM VỤ'}</button></div></article>`}).join('')}</div>`
}
function wantedView(state){
  const hasWanted=state.wanted!==null&&state.wanted!==undefined
  return `<div class="quest-summary"><span>☠ Truy nã hôm nay: <b>${state.wantedCount||0}/10</b></span><span>🔄 Làm mới sau: <b id="wanted-timer">01:00:00</b></span></div><div class="wanted-grid" role="list">${wanted.map((q,i)=>{const selected=hasWanted&&Number(state.wanted)===i;const disabled=!selected&&(hasWanted||(state.wantedCount||0)>=10);return `<article class="wanted-card${selected?' wanted-active':''}" role="listitem"><div class="wanted-head"><div class="quest-avatar">☠</div><div><div class="wanted-name">${q.name}</div><div class="wanted-badge">Boss nhỏ · ${q.level}</div></div></div><div class="wanted-map">📍 ${q.map}<br>🔎 Khả năng gặp: <b>Thấp</b></div><div class="wanted-rewards">🎁 ${q.reward}</div><div class="wanted-status">${selected?'Đang truy nã mục tiêu này...':'Mục tiêu chưa được phát hiện...'}</div><button class="quest-action${selected?' quest-cancel':''}" type="button" data-type="wanted" data-index="${i}" ${disabled?'disabled':''}>${selected?'HỦY NHIỆM VỤ':hasWanted?'KHÔNG THỂ NHẬN':'NHẬN TRUY NÃ'}</button></article>`}).join('')}</div>`
}

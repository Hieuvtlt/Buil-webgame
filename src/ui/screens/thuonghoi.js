export function ThuongHoiScreen() {
  const totalPages = 5
  const rowsPerPage = 11 // header + 11 dòng = 12 hàng tổng
  ...
  return `
    <div class="merchant-screen">
        ...
        <div class="merchant-table-wrap">
          <div class="merchant-table" role="table">
            ...
            <div class="m-row m-header">
              <div class="m-cell">Tên sản phẩm</div>
              <div class="m-cell">Cấp độ</div>
              <div class="m-cell">Phẩm cấp</div>
              <div class="m-cell">Giá</div>
            </div>

            <!-- rows placeholder (mặc định trang 1) -->
            ${Array.from({ length: rowsPerPage }, (_, i) => `
              <div class="m-row m-data" data-row="${i + 1}">
                <div class="m-cell">Item ${i + 1}</div>
      ...
      <!-- nút trang nằm dưới bảng -->
      <div class="merchant-pagination" id="merchant-pagination">
        ${Array.from({ length: totalPages }, (_, p) => `
          <button
            class="${p === 0 ? 'merchant-page-btn active' : 'merchant-page-btn'}"
            type="button"
            data-page="${p + 1}"
          >
            ${p === 0 ? 'Trang 1' : (p + 1)}
